...
TrustMark::TrustMark(TrustMark&& other) noexcept
    ...
    , modelResolutionRemove_(other.modelResolutionRemove_)
    , encoderSession_(std::move(other.encoderSession_))
    , decoderSession_(std::move(other.decoderSession_))
    , removalSession_(std::move(other.removalSession_))
...
bool TrustMark::initializeModels() {
    ...
    if (!encoderExists || !decoderExists) {
        if (verbose_) {
            ...
            std::cout << "  - " << removalPath << " " << (removalExists ? "✓" : "✗") << " (optional)" << std::endl;
        }
    ...
    if (verbose_) {
        std::cout << "Required models found. Removal model is " << (removalExists ? "available" : "not available") << "." << std::endl;
    }
    ...
    // Initialize removal session (optional)
    if (removalExists) {
        removalSession_ = std::make_unique<ONNXRuntimeSession>(removalPath, "remover");
        if (!removalSession_->isInitialized()) {
            setLastError("Failed to initialize remover: " + removalSession_->getLastError());
            return false;
        }
    } else {
        if (verbose_) {
            std::cout << "Removal model not available - watermark removal will use fallback method" << std::endl;
        }
        // Set removalSession_ to nullptr to indicate it's not available
        removalSession_ = nullptr;
    }
...
cv::Mat TrustMark::encode(const cv::Mat& coverImage, const std::string& secret,
    ...
    try {
        ...
        if (!encoderSession_ || !encoderSession_->isInitialized()) {
            if (verbose_) {
                std::cout << "Running in demo mode - simulating watermark encoding" << std::endl;
            }
            // Simulate successful encoding in demo mode by returning the original image
        ...
        cv::Mat resizedResidual = imageProcessor_->resizeImage(residual, 
                                                             croppedImage.cols, 
                                                             croppedImage.rows, 
                                                             wmMerge);
        
        // Apply residual to normalized image (both in [-1,1] range)
        cv::Mat watermarkedNormalized = normalizedImage + resizedResidual;
...
std::tuple<std::string, bool, int> TrustMark::decode(const cv::Mat& stegoImage, Mode mode) {
    try {
        ...
        if (!decoderSession_ || !decoderSession_->isInitialized()) {
            if (verbose_) {
                std::cout << "Running in demo mode - simulating watermark decoding" << std::endl;
            }
            // Simulate successful decoding in demo mode
            return {"Demo mode: Watermark decoded successfully (simulated)", true, 0};
        }
        ...
        std::vector<Ort::Value> outputs = decoderSession_->run(inputs);
...
// Remove watermark
cv::Mat TrustMark::removeWatermark(const cv::Mat& stegoImage, float wmStrength, 
                                  const std::string& wmMerge) {
    try {
        // Check if removal model is available
        if (!removalSession_) {
            if (verbose_) {
                std::cout << "Removal model not available - using fallback method" << std::endl;
            }
            // Use fallback method: return the input image (no removal performed)
            return putImageAfterProcessing(stegoImage, stegoImage);
        }
        
        if (!removalSession_->isInitialized()) {
            if (verbose_) {
                std::cout << "Running in demo mode - simulating watermark removal" << std::endl;
            }
            // Simulate successful removal in demo mode by returning the input image
            return putImageAfterProcessing(stegoImage, stegoImage);
        }
        
        // Preprocess stego image
        cv::Mat croppedImage = getImageForProcessing(stegoImage);
        cv::Mat normalizedImage = imageProcessor_->preprocessForRemoval(croppedImage, modelResolutionRemove_);
        
        // Convert to [-1, 1] range (preprocessForRemoval gives [0, 1])
        normalizedImage = normalizedImage * 2.0f - 1.0f;
        
        // Create input tensor
        std::vector<Ort::Value> inputs;
        std::vector<int64_t> imageShape = {1, 3, modelResolutionRemove_, modelResolutionRemove_};
        Ort::Value imageTensor = onnx_utils::createInputTensor(normalizedImage, "onnx::Concat_0", imageShape);
        inputs.push_back(std::move(imageTensor));
        
        // Run removal model
        std::vector<Ort::Value> outputs = removalSession_->run(inputs);
        if (outputs.empty()) {
            setLastError("Removal model failed to produce output");
            return cv::Mat();
        }
        
        // Extract cleaned image
        cv::Mat cleanedImage = onnx_utils::extractOutputTensor(outputs[0], 
                                                             {1, 3, modelResolutionRemove_, modelResolutionRemove_});
        
        // Calculate residual
        cv::Mat residual = cleanedImage - normalizedImage;
        ...
        cv::Mat denormalizedResidual = imageProcessor_->denormalizeImage(resizedResidual, -1.0f, 1.0f);
        cv::Mat denormalizedOriginal = imageProcessor_->denormalizeImage(croppedImage, 0.0f, 255.0f);
        
        cv::Mat cleanedFinal = denormalizedOriginal + wmStrength * denormalizedResidual;
        cv::threshold(cleanedFinal, cleanedFinal, 0.0, 255.0, cv::THRESH_TOZERO);
        cv::threshold(cleanedFinal, cleanedFinal, 255.0, 255.0, cv::THRESH_TRUNC);
        
        // Apply feathering and merge back to original image
        cv::Mat finalImage = putImageAfterProcessing(cleanedFinal, stegoImage);
        
        return finalImage;
        
    } catch (const std::exception& e) {
        setLastError("Watermark removal failed: " + std::string(e.what()));
        return cv::Mat();
    }
}
