...
// Preprocess for removal
cv::Mat ImageProcessor::preprocessForRemoval(const cv::Mat& image, int targetSize) {
    try {
        // Ensure image is in BGR format
        cv::Mat processedImage = image;
        if (image.channels() == 1) {
        ...
        setLastError("Preprocessing for removal failed: " + std::string(e.what()));
        return cv::Mat();
    }
}
